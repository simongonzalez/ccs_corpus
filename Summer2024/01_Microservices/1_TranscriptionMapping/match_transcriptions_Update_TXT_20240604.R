...
fls_doc = list.files('path/to/Transcriptions_TXT',
...
dfall = data.frame(doc = fls_doc, file_doc = basename(fls_doc), 
                   name = gsub('\\.txt', '', basename(fls_doc))) %>%
...
for(filei in 1:nrow(dfall)){
  ...
  df_doc <- data.frame(raw = unlist(strsplit(readtext(infile_path,
                                                      encoding = guess_encoding(infile_path)$encoding[1])$text, '\n'))) %>%
