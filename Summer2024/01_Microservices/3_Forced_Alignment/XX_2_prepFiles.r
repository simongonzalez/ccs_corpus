...
write.csv(df, 'ProcessedPhonologicalContexts.csv', row.names = F)

#Add Time in formation
dat <- df %>%
  mutate(time = onset + ((offset - onset)/2)) %>%
  mutate(row_index = 1:n()) %>%
  mutate(feats = gsub(',', ';', feats))

#Create file to be populated in Praat
dat_append <- dat %>%
  dplyr::select(speaker, time, row_index) %>%
  mutate(
    formant_1 = 0, 
...
#Create CSV files
write.csv(dat, 'ProcessedPhonologicalContexts_preped.csv', row.names = F, quote = F)
write.csv(dat_append, 'ProcessedPhonologicalContexts_append.csv', row.names = F, quote = F)
